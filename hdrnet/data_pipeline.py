...
__all__ = [
  'ImageFilesDataPipeline',  # 默认使用方式
  'StyleTransferDataPipeline',
  'HDRpDataPipeline',  # HDR+选择的方式
  ]
...
class ImageFilesDataPipeline(DataPipeline):  # 方式1
  ...
  def _produce_one_sample(self):  # 生成一个样本图片
    dirname = os.path.dirname(self.path)
    if not check_dir(dirname):
      raise ValueError("Invalid data path.")
    with open(self.path, 'r') as fid:  # 读取文本文件
    ...
    if '16-bit' in magic.from_file(input_files[0]):  # 16bit
    ...
    if os.path.splitext(input_files[0])[-1] == '.jpg':   # jpg格式
    ...
    sample['image_output'] = fullres[:, :, 3:]  # 一个样本包含4张图片
    return sample


class HDRpDataPipeline(DataPipeline):  # 方式3
  ...
  def _produce_one_sample(self):
    ...
    with open(self.path, 'r') as fid:
      flist = [l.strip() for l in fid.xreadlines()]
    input_files = [os.path.join(root, f) for f in flist if '.tfrecords' in f]  # 输入tfrecords格式
...
class StyleTransferDataPipeline(DataPipeline):  # 方式2
  def _produce_one_sample(self):
    # TODO: check dir structure
