...
def main(args, model_params, data_params):  # main函数加载模型参数和数据参数
  procname = os.path.basename(args.checkpoint_dir)
  setproctitle.setproctitle('hdrnet_{}'.format(procname))  # 进程名称
  ...
  mdl = getattr(models, args.model_name)  # 选择模型model_name  getattr(对象, 属性)返回属性值mdl
  ...
  with tf.variable_scope('train_data'):  # 训练
    train_data_pipeline = data_pipe(
        args.data_dir,  # 训练数据集目录
        shuffle=True,
        batch_size=args.batch_size, nthreads=args.data_threads,
        fliplr=args.fliplr, flipud=args.flipud, rotate=args.rotate,
        random_crop=args.random_crop, params=data_params,  # 数据参数
        output_resolution=args.output_resolution)
    train_samples = train_data_pipeline.samples  # train_samples 训练样本

  if args.eval_data_dir is not None:
    with tf.variable_scope('eval_data'):  # 验证
      eval_data_pipeline = data_pipe(
          args.eval_data_dir,  # 测试数据集目录
          shuffle=False,
          batch_size=1, nthreads=1,
          fliplr=False, flipud=False, rotate=False,
          random_crop=False, params=data_params,  # 数据参数
          output_resolution=args.output_resolution)
      eval_samples = train_data_pipeline.samples  # eval_samples 验证样本
  # ---------------------------------------------------------------------------

  # Training graph
  with tf.name_scope('train'):
    with tf.variable_scope('inference'):
      prediction = mdl.inference(  										# 模型参数  mdl调用数据集！！！
          train_samples['lowres_input'], train_samples['image_input'],
          model_params, is_training=True)
    loss = metrics.l2_loss(train_samples['image_output'], prediction)  # L2损失函数
    psnr = metrics.psnr(train_samples['image_output'], prediction)  # PSNR评价
  ...
  if args.eval_data_dir is not None:
    with tf.name_scope('eval'):
      ...
      eval_psnr = metrics.psnr(eval_samples['image_output'], prediction)  # PSNR评价
  ...
  with tf.name_scope('optimizer'):
    ...
    with tf.control_dependencies([updates]):
      opt = tf.train.AdamOptimizer(args.learning_rate)  # Adam优化器，学习率
  ...
  # Save a few graphs to tensorboard  在tensorboard显示
  summaries = [
  ...
  config.gpu_options.allow_growth = True  # Do not canibalize the entire GPU  配置GPU训练
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser()  # 设置训练参数

  # pylint: disable=line-too-long
  # ----------------------------------------------------------------------------
  req_grp = parser.add_argument_group('required')
  req_grp.add_argument('checkpoint_dir', default=None, help='directory to save checkpoints to.')  # 保存模型的位置
  req_grp.add_argument('data_dir', default=None, help='input directory containing the training .tfrecords or images.')  # 训练集，tfrecords或图片格式
  req_grp.add_argument('--eval_data_dir', default=None, type=str, help='directory with the validation data.')

  # Training, logging and checkpointing parameters  训练参数
  ...
  # Data pipeline and data augmentation  数据参数
  ...
  model_grp.add_argument('--model_name', default=models.__all__[0], type=str, help='classname of the model to use.', choices=models.__all__)  # 选择模型，默认HDRNetCurves
  model_grp.add_argument('--data_pipeline', default='ImageFilesDataPipeline', help='classname of the data pipeline to use.', choices=dp.__all__)  # 选择导入数据方式，默认ImageFilesDataPipeline
  ...
  model_params = {}  # 模型参数
  for a in model_grp._group_actions:  # model_grp是上面的模型参数
    model_params[a.dest] = getattr(args, a.dest, None)

  data_params = {}  # 数据参数
  for a in data_grp._group_actions:  # data_grp是上面的数据参数
