...
__all__ = [  # 有以下3个模型选择
  'HDRNetCurves',
  'HDRNetPointwiseNNGuide',  # hdrp选择这种模型
  'HDRNetGaussianPyrNN',
]


class HDRNetCurves(object):  # 模型1
  ...
  def inference(cls, lowres_input, fullres_input, params,
    ...
    with tf.variable_scope('coefficients'):
      bilateral_coeffs = cls._coefficients(lowres_input, params, is_training)  # lowres_input用于训练双边网格的系数
      tf.add_to_collection('bilateral_coefficients', bilateral_coeffs)

    with tf.variable_scope('guide'):
      guide = cls._guide(fullres_input, params, is_training)  # fullres_input用于指导图和输出
  ...
  @classmethod
  def _guide(cls, input_tensor, params, is_training):  # 生成指导图
    npts = 16  # number of control points for the curve
    nchans = input_tensor.get_shape().as_list()[-1]  # 对输入数据变形

    guidemap = input_tensor

    # Color space change  色域转换
  ...
  @classmethod
  def _output(cls, im, guide, coeffs):  # 生成输出
    with tf.device('/gpu:0'):
      out = bilateral_slice_apply(coeffs, guide, im, has_offset=True, name='slice')
    return out


class HDRNetPointwiseNNGuide(HDRNetCurves):  # 模型2
...
class HDRNetGaussianPyrNN(HDRNetPointwiseNNGuide):  # 模型3
